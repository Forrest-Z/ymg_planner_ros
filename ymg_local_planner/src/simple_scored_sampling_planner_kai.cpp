...
namespace base_local_planner {
  ...
  SimpleScoredSamplingPlannerKai::SimpleScoredSamplingPlannerKai(std::vector<TrajectorySampleGenerator*> gen_list, std::vector<TrajectoryCostFunction*>& critics, int max_samples) {
    max_samples_ = max_samples;
    gen_list_ = gen_list;
    critics_ = critics;
  }

  double SimpleScoredSamplingPlannerKai::scoreTrajectory(Trajectory& traj, double best_traj_cost) {
    double traj_cost = 0;
    int gen_id = 0;
  ...
  bool SimpleScoredSamplingPlannerKai::findBestTrajectory(Trajectory& traj, std::vector<Trajectory>* all_explored) {
